...
const Wishlist = () => {
  ...
  useEffect(() => {
    const fetchWishlist = async () => {
      try {
        const token = localStorage.getItem('authToken'); // Ensure JWT token is stored in localStorage
        if (!token) {
        ...
        if (response.data && response.data.wishlist) {
          ...
          // If no wishlist items, set empty array without error
          setWishlistItems([]);
          console.error('No items:', error);
        }
      } catch (error) {
        console.error('No items in wishlist:', error);
